...
if [ "$uid" != "" ]; then
    ...
    #/usr/local/bin/cap_fowner /usr/bin/setfacl -R -m $uid:rwx $pdir && \
    #smbcacls //dccn-ceph003/project_cephfs_k /$pid -M "ACL:DCCN\\${uid}:ALLOWED/3/CHANGE" -U"DCCN\\${me}"
    #smbcacls //dccn-ceph003/project_cephfs_k /$pid -M "ACL:DCCN\\${uid}:ALLOWED/3/CHANGE" -U"DCCN\\${me}" -I allow --propagate-inheritance
fi
